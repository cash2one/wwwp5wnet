function related(stockcode) {
    ...
    loadJs('http://www.p5w.net/jsjdpn/' + stockcode + '.js',
		function () {
		    try {
		        ...
		        $('<li><a href="http://company.p5w.net/gszl/qgqp.asp?zqdm=' + stockcode + '" target="_blank">' + qgqptext + '</a>\u66f4\u65b0\u65e5\u671f\uff1a' + qgqpdate + '</li>').appendTo($('#qgqp ul'));
		    } catch (e) { }
		},
		'gb2312'
	);
    ...
    $.getJSON(jsonUrl, function (data) {
        ...
        $(data.lsgg).each(function (i, e) {
            var pid = e.link.replace(/.*(\/|\\)/, "").replace(/(\.\w+)$/, "");
            ...
            var title = e.title || '';
            title = title.sub(71, "...");
            $('<li><a href="' + ggUrl + '" target="_blank">' + title + '</a>' + pubdate.replaceAll('-', '/') + '</li>').appendTo($('#contentgg ul'));
        });
        $('<a href="http://company.p5w.net/gszl/lsgg.asp?zqdm=' + stockcode + '" target="_blank">\u66f4\u591a\u516c\u544a\u2026</a>').appendTo($('#moregg'));
        ...
        $(data.fxpj).each(function (i, e) {
            ...
            if (pj1.length > 0) {
                tzpjhtml += '  <tr>';
                tzpjhtml += '    <td>' + date + '</td>';
                tzpjhtml += '    <td><a href="http://company.p5w.net/gszl/fxpj.asp?zqdm=' + code + '" target="_blank">' + code + '</a></td>';
                tzpjhtml += '    <td><a href="http://company.p5w.net/gszl/fxpj.asp?zqdm=' + code + '" target="_blank">' + name + '</a></td>';
                tzpjhtml += '    <td>' + orgname + '</td>';
                if (bd == 'K') {
                    tzpjhtml += '    <td><a href="http://company.p5w.net/gszl/fxpj.asp?zqdm=' + code + '" target="_blank">' + pjbd[bd] + ' "' + pj2 + '"</a></td>';
                }
                else {
                    tzpjhtml += '    <td><a href="http://company.p5w.net/gszl/fxpj.asp?zqdm=' + code + '" target="_blank">"' + pj2 + '" ' + pjbd[bd] + ' "' + pj1 + '"</a></td>';
                }
                tzpjhtml += '  </tr>';
            }
        });
