...
$(function () {
    ...
    $.getJSON("http://data.p5w.net/yb/p5wstock.php?m=ggql&count=8&callback=?", function (data) {
        if (data.ggql != null) {
            var ul = $('#fxpj-list');
            var ultitle = $('.title', ul);
            ul.empty().append(ultitle);
            $(data.ggql).each(function (i) {
                if (i > 7) return false;
                var title = this.s2;
                $('<li>' + this.s1 + '</li><li><a href="' + hqUrl + parseCode(this.s1) + '" target="_blank">' + title + '</a></li><li class="red">' + this.s8 + '</li><li>' + this.s9 + '%</li>').appendTo(ul);
            });
        }
    });
