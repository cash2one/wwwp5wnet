...
(function () {
    ...
    function loadJs(url, callback, chartset) {
        if (!chartset) chartset = "utf-8";
        var done = false;
    ...
    easyloader = {
        ...
        load: function (name, callback, chartset) {
            if (!chartset) chartset = "utf-8";
            if (/\.css$/i.test(name)) {
                if (/^http/i.test(name)) {
                ...
                if (/^http/i.test(name)) {
                    loadJs(name, callback, chartset);
                } else {
                    loadJs(easyloader.base + name, callback, chartset);
                }
            } else {
