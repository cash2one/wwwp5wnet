...
function related(stockcode) {
    ...
    $.getJSON(jsonUrl, function (data) {
        ...
        $(data.lsgg).each(function (i, e) {
            var pid = e.s1;
            var ggUrl = 'http://data.p5w.net/t' + pid + '.html';
            var pubdate = e.s4 || '';
            var title = e.s3 || '';
            title = title.sub(71, "...");
            $('<li><a href="' + ggUrl + '" target="_blank">' + title + '</a>' + pubdate.replaceAll('-', '/') + '</li>').appendTo($('#contentgg ul'));
        });
        $('<a href="http://data.p5w.net/stock/lsgg.php?code=' + parseCode(stockcode) + '" target="_blank">\u66f4\u591a\u516c\u544a\u2026</a>').appendTo($('#moregg'));
        ...
        $('.cname').siblings('.more').eq(1).find('a').attr('href', qgqpLinkMore);
        $('.cname').siblings('.more').eq(2).find('a').attr('href', tzpjLink);
...
$(function () {
    var code = $('.rq-tab-1 input').attr('value').split(';')[0];
    if (code != null) {
        $('.rq-tab-1 input').attr('value', code);
        related(code);
    }
    hotwords();
});