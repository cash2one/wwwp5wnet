	...
	p5w = global.p5w = {
	    ...
	    scrollTo: (function () {
	        ...
	        return function (top, callback) {
				...
				fx = function () {
				    ...
				    if (tMap >= duration) {
				        setTop(top);
				        return (callback || p5w.noop).call(p5w, top);
				    }
	    ...
	    loadScript: function (url, callback, args) {
	        ...
	        if (!cache.loaded) {
	            ...
	            (callback || Base.noop).call(null, url);
	        }
	        return this;
	    },
	    //Cookies
	    ...
	    is_ie6: !-[1, ] && !global.XMLHttpRequest,
...
;(function(global){var p5w=global.p5w||(global.p5w={});var rarg1=/\$1/g,rgquote=/\\"/g,rbr=/([\r\n])/g,rchars=/(["\\])/g,rdbgstrich=/\\\\/g,rfuns=/<%\s*(\w+|.)([\s\S]*?)\s*%>/g,rbrhash={'10':'n','13':'r'},helpers={'=':{render:'__.push($1);'}};p5w.tmpl=function(tmpl,data){var render=new Function('_data','var __=[];__.data=_data;'+'with(_data){__.push("'+tmpl.replace(rchars,'\\$1').replace(rfuns,function(a,key,body){body=body.replace(rbr,';').replace(rgquote,'"').replace(rdbgstrich,'\\');var helper=helpers[key],tmp=!helper?key+body:typeof helper.render==='function'?helper.render.call(ds,body,data):helper.render.replace(rarg1,body);return'");'+tmp+'__.push("';}).replace(rbr,function(a,b){return'\\'+(rbrhash[b.charCodeAt(0)]||b);})+'");}return __.join("");');return data?render.call(data,data):render;};p5w.tmpl.helper=helpers;})(this);
...
$(function () {
    ...
    p5w.loadScript("http://www.p5w.net/js/quick_links.js", p5w.noop, {"charset":"utf-8"});
...
function foot14() {
    ...
    document.writeln('        <div class="c-logo mt20"><img src="/images14/logo-gray.jpg" width="143" height="54" alt="全景网" /></div>');
    ...
    document.writeln('            <p class="fontArial">Copyright &copy; 2000-2015 Panorama Network Co., Ltd, All Rights Reserved</p>');
    document.writeln('            <p class="mt10">全景网络有限公司版权所有</p>');
    document.writeln('            <p>未经全景网书面授权，请勿转载内容或建立镜像，违者依法必究！</p>');
    document.writeln('        </div>');
    document.writeln('        <div class="c-img"><img src="/images14/c-img.jpg" width="347" height="103" alt="全景网" /></div>');
    ...
    document.write(unescape("%3Cspan id='cnzz_stat_icon_3475086'%3E%3C/span%3E%3Cscript src='" + cnzz_protocol + "s5.cnzz.com/stat.php%3Fid%3D3475086%26show%3Dpic' type='text/javascript'%3E%3C/script%3E"));
    document.writeln('    </div>');
    document.writeln('</div>');
