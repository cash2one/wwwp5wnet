...
function LoadLatest() {
    ...
    this.loadData = function (p) {
        ...
        for (var i = 0 + LoadL.pageSize * (p - 1) ; i < LoadL.pageSize * p && i < LoadL.rows.length; i++) {
            ...
            $('<li>' + code + '<span class="' + tc + '"><a href="../contents.htm?id='+encodeURIComponent(filename)+'&f='+f+'" target="_blank" title="' + item[2] + '">' + title + '</a>' + img + '</span><span class="time">' + item[6] + '</span></li>').appendTo(ul);
